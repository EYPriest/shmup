LevelJunkerTest = Class{ __includes = Level}

function LevelJunkerTest:init()
  ...
  self.droid_time_accumulator = 0
  self.droid_spawn_time = 1
  
  self.bird_ship_time_accumulator = 0
  self.bird_ship_spawn_time = 4
  
end


function LevelJunkerTest:update(dt)
  
  Level.update(self,dt)
  
   --Create Droids
  self.droid_time_accumulator = self.droid_time_accumulator + dt
  if ( self.droid_time_accumulator > self.droid_spawn_time ) then
    self.droid_time_accumulator = self.droid_time_accumulator - self.droid_spawn_time
    local new_enemy = EnemyJunkerDroid( Vector(love.graphics.getWidth() + 1, love.math.random( love.graphics.getHeight() - EnemyJunkerDroid.height ) ) )
    table.insert( enemies_alive, new_enemy )
  end
  
  --Create Bird Ships
  local tempHeight = 20
  self.bird_ship_time_accumulator = self.bird_ship_time_accumulator + dt
  if ( self.bird_ship_time_accumulator > self.bird_ship_spawn_time ) then
    self.bird_ship_time_accumulator = self.bird_ship_time_accumulator - self.bird_ship_spawn_time
    table.insert( enemies_alive, EnemyBird( Vector(love.graphics.getWidth() + 1, love.math.random(love.graphics.getHeight() - tempHeight ) ) ) )
  end
  ...
  if ( asteroid_time_accumulator > asteroid_spawn_time ) then
    ...
    table.insert( enemies_alive, EnemyJunkerDroid( Vector(love.graphics.getWidth() + 1, love.math.random(love.graphics.getHeight() * 1) ) ) )
  end
...
function LevelJunkerTest:change_size()
  ...
  elseif ( self.size == 3 ) then
    ...
    self.backgrounds[1] = {}
    self.backgrounds[1].image = love.graphics.newImage("res/Backgrounds//Industrial4-3X-bg.png")
    self.backgrounds[1].pos_x = 0
    self.backgrounds[1].speed_x = 1
    self.backgrounds[1].scale = forest2scale
    
    self.backgrounds[2] = {}
    self.backgrounds[2].image = love.graphics.newImage("res/Backgrounds//Industrial4-3X-far.png")
    self.backgrounds[2].pos_x = 0
    self.backgrounds[2].speed_x = 2
    self.backgrounds[2].scale = forest2scale
    
    self.backgrounds[3] = {}
    self.backgrounds[3].image = love.graphics.newImage("res/Backgrounds//Industrial4-3X-mid.png")
    self.backgrounds[3].pos_x = 0
    self.backgrounds[3].speed_x = 3
    self.backgrounds[3].scale = forest2scale
    
    self.backgrounds[4] = {}
    self.backgrounds[4].image = love.graphics.newImage("res/Backgrounds//Industrial4-3X-close.png")
    self.backgrounds[4].pos_x = 0
    self.backgrounds[4].speed_x = 4
    self.backgrounds[4].scale = forest2scale
